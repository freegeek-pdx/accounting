...
if tail -1 output.sql | grep -q ERROR ; then
    sed -i '/ERROR: / d' output.sql
    psql --quiet xtuple < output.sql 
    ./allocations/gen_sql.rb  $PERIOD_ENDDATE $COSTPOOL_CODE input.csv "$ALLOCATION_FILE" output.csv > output.sql
    psql --quiet xtuple < output.sql 
else
    psql --quiet xtuple < output.sql 
fi 
